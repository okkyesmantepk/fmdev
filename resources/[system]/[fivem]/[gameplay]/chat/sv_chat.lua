...
RegisterCommand("disable", function(source, args)
    ...
    exports.oxmysql:execute("SELECT rank FROM users WHERE hex_id = ?", {steamIdentifier}, function(data)
        if data[1].rank ~= "user" then
            if command == "ooc" then
                toggled = not toggled
                TriggerClientEvent("DoLongHudText", pSrc, ('OOC has been %s!'):format(toggled and 'Disabled' or 'Enabled')) -- Not your chat u bozo
            end
        end
    end)
end)
...
AddEventHandler('chat:me_log', function(pMeText)
    ...
    PerformHttpRequest(Me_ERP_Log, function(err, text, headers) end, 'POST', json.encode({username = "NopixelRP",  avatar_url = "https://cdn.discordapp.com/attachments/XXX/XXXX/Untitled-1.png",embeds = connect}), { ['Content-Type'] = 'application/json' })
end)