...
AddEventHandler('police:jailing', function(args)
    ...
    TriggerServerEvent('np-jail:pJailedLog', tonumber(args[2]), tonumber(args[1]))
end)
...
exports["np-polyzone"]:AddBoxZone("evidence_zone", vector3(474.26, -1005.19, 26.27), 1, 3.0, {
    heading=0,
    --debugPoly=false,
    minZ=23.27,
    maxZ=27.27
...
AddEventHandler('np-polyzone:enter', function(pZone)
    if pZone == "evidence_zone" then
        inEvidenceZone = true
        exports['np-interface']:showInteraction('/evidence [LOCKER ID]')
    end
end)
...
AddEventHandler('police:tenThirteenA', function()
    exports["np-dispatch"]:policedead('10-13A', "Officer Down URGENT", "630", true)
end)

RegisterNetEvent('police:tenThirteenB')
AddEventHandler('police:tenThirteenB', function()
    exports["np-dispatch"]:policedead('10-13B', "Officer Down", "630", false)
end)

RegisterNetEvent("police:tenForteenA")
AddEventHandler("police:tenForteenA", function()
    exports["np-dispatch"]:policedead('10-14A', "Medic Down URGENT", "630", true)
end)

RegisterNetEvent("police:tenForteenB")
AddEventHandler("police:tenForteenB", function()
    local pos = GetEntityCoords(PlayerPedId(), true)
    exports["np-dispatch"]:policedead('10-14B', "Medic Down", "630", false)
end)
...
RegisterInterfaceCallback('police:hireNewPersonel', function(data, cb)
    ...
    if dept == 1 then
        TriggerServerEvent('np-police:hire_pd', data.values.cid, data.values.rank, data.values.callsign, 'police')
    elseif dept == 2 then
        TriggerServerEvent('np-police:hire_pd', data.values.cid, data.values.rank, data.values.callsign, 'sheriff')
    elseif dept == 3 then
        TriggerServerEvent('np-police:hire_pd', data.values.cid, data.values.rank, data.values.callsign, 'state')
    end
end)
...
AddEventHandler('police:hire', function()
    TriggerServerEvent('np-police:sv:open_hire_menu')
end)

exports["np-polytarget"]:AddCircleZone("np_police_hire",  vector3(463.28, -984.29, 30.72), 0.55, {
    useZ = true
})

exports["np-interact"]:AddPeekEntryByPolyTarget("np_police_hire", {{
    event = "police:hire",
    id = "np_police_hire",
    icon = "id-badge",
    label = "Hire Personel!",
...
AddEventHandler('police:BillLog', function(pAmt, pPlayer)
    TriggerServerEvent('np-jail:pBilledLog', pAmt, pPlayer)
end)
...
exports["np-polyzone"]:AddBoxZone("np_police_swat_armory", vector3(487.51, -997.09, 30.69), 1, 1.8, {
    heading=270,
    --debugPoly=false,
    minZ=27.09,
    maxZ=31.09
