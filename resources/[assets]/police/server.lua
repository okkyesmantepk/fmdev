...
RegisterCommand('bill', function(source, args)
    ...
    exports.ghmattimysql:execute('SELECT job FROM jobs_whitelist WHERE cid = ?', {character2.id}, function(result)
        if result[1].job == 'police' then
            ...
            TriggerServerEvent('np-financials:business_money', args[2], 'police', 'add')
        end
    end)
end)
...
AddEventHandler("911", function(args, pStreetName)
    ...
    PerformHttpRequest("https://discord.com/api/webhooks/ENTER_HERE", function(err, text, headers) end, 'POST', json.encode({username = "np | 911 Call", embeds = connect, avatar_url = "https://i.imgur.com/hMqEEQp.png"}), { ['Content-Type'] = 'application/json' })
...
AddEventHandler("311", function(args)
    ...
    TriggerClientEvent("chatMessage", src,
        ...
        PerformHttpRequest("https://discord.com/api/webhooks/ENTER_HERE", function(err, text, headers) end, 'POST', json.encode({username = "np | 311 Call", embeds = connect, avatar_url = "https://i.imgur.com/hMqEEQp.png"}), { ['Content-Type'] = 'application/json' })
...
RegisterServerEvent('np-police:hire_pd')
AddEventHandler('np-police:hire_pd', function(pCID, pRank, pCallsign, pDept)
...
RegisterServerEvent('np-police:sv:open_hire_menu')
AddEventHandler('np-police:sv:open_hire_menu', function()
    local src = source
    local user = exports["np-base"]:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
...
local JSON = json.decode(LoadResourceFile("np-hud", "/config/pursuitModes.json"))
