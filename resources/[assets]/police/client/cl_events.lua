...
RegisterInterfaceCallback("np-police:handler", function(data, cb)
...
AddEventHandler('np-inventory:itemUsed', function(itemId, itemInfo, inventoryName, slot)
    ...
    if not metaData.inventoryId then
        exports['np-interface']:openApplication('textbox', {
            callbackUrl = 'np-interface:saveEvidenceBag',
            key = {
                slot = slot
            },
            items = {{
...
RegisterInterfaceCallback("np-interface:saveEvidenceBag", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
