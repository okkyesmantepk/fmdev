...
RegisterInterfaceCallback("np-weed:addFertilizer", function (pParams)
    ...
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
            local success = RPC.execute("np-weed:addFertilizer", pParams.key.id, pParams.key.type)
        if not success then
...
RegisterInterfaceCallback("np-weed:addMaleSeed", function (pParams)
    ...
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        RPC.execute("np-weed:addMaleSeed", pParams.key.id)
        TriggerEvent("inventory:removeItem", "maleseed", 1)
    end
    showPlantMenu(pParams.key.id)
end)

RegisterInterfaceCallback("np-weed:removePlant", function (pParams)
    TriggerEvent("animation:PlayAnimation","layspike")
    local finished = exports["np-taskbar"]:taskBar(3000, "Removing", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        local getFertilizer = getPlantGrowthPercent(weedPlants[pParams.key.id]) > 20.0
        local success = RPC.execute("np-weed:removePlant", pParams.key.id, getFertilizer)
        if not success then
...
AddEventHandler("np-weed:pickPlant", function(pContext, pEntity)
    ...
    TriggerEvent("animation:PlayAnimation", "layspike")
    local finished = exports["np-taskbar"]:taskBar(10000, "Harvesting", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
