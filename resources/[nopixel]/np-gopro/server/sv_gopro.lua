RPC.register("np-gopros:addSecCamera", function(pSource, pName, pCoords, pRotation)
...
RPC.register("np-gopros:getSecurityCameraById", function(pSource, pCameraID)
    local result = MySQL.query.await([[
...
RPC.register("np-gopros:getSecurityCameraByStateID", function(pSource, pCid)
...
RPC.register("np-gopros:getSecurityCameraAccessByCameraID", function(pSource, pCamera)
...
RPC.register("np-gopros:addUserToCamera", function(pSource, pCamera, pCid)
    local camera = MySQL.query.await([[
        SELECT * FROM _cameras WHERE id = ?
    ]], {pCamera})

    print(pCid)
    local pCid = exports['np-phone']:getCIDFromPhoneNumber(pCid)
    print(pCid)
...
RPC.register("np-gopros:removeUserFromCamera", function(pSource, pCamera, pCid)
    local camera = MySQL.query.await([[
        SELECT * FROM _cameras WHERE id = ?
    ]], {pCamera})
