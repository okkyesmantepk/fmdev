...
AddEventHandler("np-gopros:updateRegisteredCams", function(pDashcams)
  ...
  exports["np-interface"]:sendAppEvent("gopros", { dashcams = activeDashcams })
end)
...
RegisterCommand("np-gopros:addUserToCamera", function()
  RPC.execute("np-gopros:addUserToCamera",1,868)
  
  end)
  

--[[
RegisterUICallback("np-interface:goproChangeSelectedPov", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = 'done' }})
  local activateGoPro = activateGoPro(tonumber(data.netId))
  Wait(600)
  exports["np-interface"]:sendAppEvent("gopros", { switchingViews = false })
end)
]]
...
AddEventHandler("np-interface:application-closed", function(name)
  ...
  exports["np-interface"]:sendAppEvent("hud", { display = true })
  DoScreenFadeIn(1000)
end)
          ...
          if IsControlJustReleased(0, 38) then
              ...
              if destinationSelected then
                ...
                exports['np-interface']:openApplication('textbox', {
                  callbackUrl = 'np-interface:goProRegisterSecCam',
                  key = 1,
                  items = {
                    {
                      icon = "pencil-alt",
                ...
                Wait(100)
                exports["np-interface"]:showInteraction(("[E] %s"):format("View Here"))
...
AddEventHandler("np-inventory:itemUsed", function(item)
  ...
  local prompt = exports["np-interface"]:OpenInputMenu({
    {
      name = 'name',
      label = 'Security Camera Stream Name',
      icon = 'pencil-alt'
  ...
  local success = RPC.execute("np-gopros:addSecCamera", prompt.name, result[2].coords, result[2].rotation)
  if success then
...
local function activateSecurityCameraById(pId)
  local cam = RPC.execute("np-gopros:getSecurityCameraById", pId)
  ...
  exports["np-interface"]:sendAppEvent("hud", { display = false })
  DoScreenFadeIn(1000)
end
...
--[[
RegisterUICallback("np-interface:gopros:getSecurityCameras", function(data, cb)
  local cid = exports["isPed"]:isPed("cid")
  local cams = RPC.execute("np-gopros:getSecurityCameraByCid", cid)
  cb({ data = cams, meta = { ok = true, message = "done" } })
end)

RegisterUICallback("np-interface:gopros:viewCameraById", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  Wait(500)
  exports["np-interface"]:closeApplication("phone")
...
--[[
RegisterUICallback("np-interface:gopros:addUserToCamera", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  RPC.execute("np-gopros:addUserToCamera", data.camera, data.cid)
end)
