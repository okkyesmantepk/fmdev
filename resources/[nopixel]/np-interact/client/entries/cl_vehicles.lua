...
Entries[#Entries + 1] = {
    ...
    options = {
        distance = { radius = 1.8 },
        isEnabled = function(pEntity, pContext)
            return exports['np-carbombs']:isThisVehicleABomb(NetworkGetNetworkIdFromEntity(pEntity))
        end
    }
}
...
Entries[#Entries + 1] = {
    ...
    options = {
        distance = { radius = 1.8 },
        isEnabled = function(pEntity, pContext)
            return exports['np-activities']:canHandOffPackages() and exports['np-inventory']:hasEnoughOfItem('darkmarketpackage', 1)
        end
    }
}
...
--             icon = "car-crash",
--             event = 'np-carscrap:ScrapVehicle',
--             parameters = {}
--         }
--     },
--     options = {
--         distance = { radius = 3.0 },
--         isEnabled = function(pEntity, pContext)
--             return exports['np-carscrap']:InScrapLocation()
--         end
--     }
-- }
...
--             label = "Chop vehicle",
--             icon = "car-crash",
--             event = 'np-carscrap:StartCarScrappingProcess',
--             parameters = {}
--         }
--     },
--     options = {
--         distance = { radius = 3.0 },
--         isEnabled = function(pEntity, pContext)
--             return exports['np-carscrap']:InNormalScrapLocation()
--         end
--     }
-- }
    ...
    data = {
        {
            ...
            label = "Scratch Vin",
            icon = "eye-slash",
            event = "np-boosting:finish_scratch",
            parameters = {}
        }
    },
    options = {
        ...
        isEnabled = function(pEntity, pContext)
            return (not (isCloseToHood(pEntity, PlayerPedId()) or isCloseToBoot(pEntity, PlayerPedId(), 2.0, pContext.model))
              or pContext.model == GetHashKey("emsnspeedo"))
              and not IsPedInAnyVehicle(PlayerPedId(), false)
              and NetworkGetEntityIsNetworked(pEntity) and exports['np-boosting']:pCanFinishScratch()
        end
    }
}
