...
RegisterServerEvent('np-jacksons_autos:CarTable')
AddEventHandler('np-jacksons_autos:CarTable', function(table)
    if table ~= nil then
        carTable = table
        TriggerClientEvent('np-jacksons_autos:ReturnCarTable', -1, carTable)
        for i=1, #carTable do
...
RegisterServerEvent('np-jacksons_autos:FinaceEnabledSV')
AddEventHandler('np-jacksons_autos:FinaceEnabledSV', function(plate)
    if plate ~= nil then
        TriggerClientEvent('np-jacksons_autos:FinaceEnabledCL', -1, plate)
    end
end)

RegisterServerEvent('np-jacksons_autos:BuyEnabledSV')
AddEventHandler('np-jacksons_autos:BuyEnabledSV', function(plate)
    if plate ~= nil then
        TriggerClientEvent('np-jacksons_autos:BuyEnabledCL', -1, plate)
    end
end)

-- return table
-- TODO (return db table)
RegisterServerEvent('np-jacksons_autos:RequestCarTable')
AddEventHandler('np-jacksons_autos:RequestCarTable', function()
    local user = source
    exports.oxmysql:execute("SELECT * FROM vehicle_display", {}, function(display)
        ...
        TriggerClientEvent('np-jacksons_autos:ReturnCarTable', user, carTable)
    end)
end)

-- Check if player has enough money
RegisterServerEvent('np-jacksons_autos:ChechMoney')
AddEventHandler('np-jacksons_autos:ChechMoney', function(name, model, price)
    local src = source
    ...
    elseif tonumber(cash) <= price then
        ...
        TriggerClientEvent('np-jacksons_autos:FailedPurchase', src)
    end
end)

RegisterServerEvent('np-jacksons_autos:BuyVehicle')
AddEventHandler('np-jacksons_autos:BuyVehicle', function(plate, name, vehicle, price, personalvehicle)
    local src = source
    local user = exports["np-base"]:getModule("Player"):GetUser(src)
