...
function updateCarTable(model,price,name)
	for i=1, #carTable do
		...
		TriggerServerEvent("np-jacksons_autos:CarTable",carTable)
	end
end

local myspawnedvehs = {}

RegisterNetEvent("np-jacksons_autos:testdrive")
AddEventHandler("np-jacksons_autos:testdrive", function()
	if exports["np-business"]:IsEmployedAt('jacksons_autos') or #(vector3(-775.9081, -1032.289, 13.719951) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
RegisterNetEvent("np-jacksons_autos:EnableBuy")
AddEventHandler("np-jacksons_autos:EnableBuy", function()
	if #(vector3(-775.4795, -1032.456, 14.129936) - GetEntityCoords(PlayerPedId())) > 50.0 and exports["np-business"]:IsEmployedAt('jacksons_autos') then
		return
	end
	...
	TriggerServerEvent("np-jacksons_autos:BuyEnabledSV",vehplate)
end)

RegisterNetEvent("np-jacksons_autos:BuyEnabledCL")
AddEventHandler("np-jacksons_autos:BuyEnabledCL", function(addplate)
	buyPlate[addplate] = true
	Citizen.Wait(60000)
	buyPlate[addplate] = nil
end)

RegisterNetEvent("np-jacksons:CommissionPrice")
AddEventHandler("np-jacksons:CommissionPrice", function(newAmount)
	if eexports["np-business"]:IsEmployedAt('jacksons_autos') or #(vector3(-775.4795, -1032.456, 14.129936) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
RegisterNetEvent("np-jacksons_autos:ReturnCarTable")
AddEventHandler("np-jacksons_autos:ReturnCarTable", function(newTable)
	carTable = newTable
	DespawnSaleVehicles()
	SpawnSaleVehicles()
end)
...
function AttemptBuy(tableid)
	...
	TriggerServerEvent('np-jacksons_autos:ChechMoney',name, model, price)
...
function DrawPrices()
	for i = 1, #pSpawnLocations do
		if #(vector3(pSpawnLocations[i]["x"],pSpawnLocations[i]["y"],pSpawnLocations[i]["z"]) - GetEntityCoords(PlayerPedId())) < 1.5 then
			...
			if exports["np-business"]:IsEmployedAt('jacksons_autos') then
...
function SpawnSaleVehicles()
	if not hasspawned then
		TriggerServerEvent("np-jacksons_autos:RequestCarTable")
		Citizen.Wait(1500)
	end
...
function CloseCreator(name, veh, price, plate)
	Citizen.CreateThread(function()
		...
		if not boughtcar then
			...
			local VehicleProps = exports['np-bennys']:FetchVehProps(personalvehicle)
			...
			TriggerServerEvent('np-jacksons_autos:BuyVehicle', plate, vehname, vehicle, price, VehicleProps)
			TriggerServerEvent('np-cardealer:log', exports['isPed']:isPed('cid'), vehname, price, plate, 'Jacksons Autos')
			TriggerServerEvent('np-financials:business_money', price / 10, 'jacksons_autos', 'add')
			exports['np-interface']:hideInteraction()
			DespawnSaleVehicles()
			SpawnSaleVehicles()
		end
...
RegisterNetEvent("np-jacksons_autos:FailedPurchase")
AddEventHandler("np-jacksons_autos:FailedPurchase", function()
...
function ButtonSelected(button)
	...
	elseif this == "b-class" or this == "s-class" then
		if ownerMenu then
			...
			TriggerServerEvent('np-jacksons_autos:ChechMoney',button.name, button.model, button.costs)
		end
	end
end
	...
	while true do
		...
		if vehshop.opened then
			...
			for i,button in pairs(menu.buttons) do
				...
				if exports["np-business"]:IsEmployedAt('jacksons_autos') and i >= vehshop.menu.from and i <= vehshop.menu.to then
...
RegisterCommand('commission', function(source, args, raw)
	if exports["np-business"]:IsEmployedAt('jacksons_autos') then
		...
		if tonumber(amount) > 0 and tonumber(amount) <= 60 then
			TriggerEvent('np-jacksons_autos:CommissionPrice', amount)
		else
...
RegisterCommand('testdrive', function(source, args, raw)
	if exports["np-business"]:IsEmployedAt('jacksons_autos') then
		TriggerEvent('np-jacksons_autos:testdrive')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
	end
end)
...
RegisterCommand('enableBuy', function(source, args, raw)
	if exports["np-business"]:IsEmployedAt('jacksons_autos') then
		TriggerEvent('np-jacksons_autos:EnableBuy')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
	end
end)