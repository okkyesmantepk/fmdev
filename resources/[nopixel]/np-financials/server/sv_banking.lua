...
RegisterServerEvent('np-financials:addMoney')
AddEventHandler('np-financials:addMoney', function(amount)
    local src = source
    local user = exports["np-base"]:getModule("Player"):GetUser(src)
    user:addMoney(tonumber(amount))
end)

RegisterServerEvent('np-financials:removeMoney')
AddEventHandler('np-financials:removeMoney', function(amount)
    local src = source
    local user = exports["np-base"]:getModule("Player"):GetUser(src)
    user:removeMoney(tonumber(amount))
end)

RegisterServerEvent("np-financials:cash:get")
AddEventHandler('np-financials:cash:get', function(amount)

    local src = source
    local user = exports["np-base"]:getModule("Player"):GetUser(src)
    local cash = user:getCash()
    TriggerClientEvent("np-financials:cash",cash)
end)

RegisterServerEvent("np-financials:business_money", function(amt, account, case)
...
RegisterServerEvent('np-financials:deposit_log')
AddEventHandler('np-financials:deposit_log', function(pDepositAmt, pDepositComment)
...
RegisterServerEvent('np-financials:withdraw_log')
AddEventHandler('np-financials:withdraw_log', function(pWithDrawAmt, pWithDrawComment)
    local pName = GetPlayerName(source)
    local connect = {
        {
            ["color"] = "16777215",
