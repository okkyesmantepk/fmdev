RegisterNetEvent('np-heists:checkAvailability')
AddEventHandler('np-heists:checkAvailability', function() 
    ...
    exports["np-interface"]:showContextMenu(FatCunt)
end)

RegisterNetEvent('np-heists:purchaseHeistEquipment')
AddEventHandler('np-heists:purchaseHeistEquipment', function()
    local HeistEquipment = {
		{
            ...
            children = {
                ...
                action = "np-heists:purchaseGreenLaptop",
            },
        },
        {
            title = "Paleto Laptop",
            ...
            children = {
                ...
                action = "np-heists:purchaseBlueLaptop",
            },
        },
    }
...
RegisterInterfaceCallback('np-heists:purchaseGreenLaptop', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    if exports['np-inventory']:hasEnoughOfItem('heistusb5', 1) then
        TriggerEvent('inventory:removeItem', 'heistusb5', 1)
        TriggerEvent("player:receiveItem", "heistlaptop3", 1)
    end
end)

RegisterInterfaceCallback('np-heists:purchaseBlueLaptop', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
