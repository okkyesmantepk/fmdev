    ...
    if pd >= 0 then
        if distance < 1.5 then
            if exports["np-inventory"]:hasEnoughOfItem("thermitecharge", 1) then
                frontdoor()
                TriggerServerEvent('np-heists:bobcatLog')
                TriggerEvent("inventory:removeItem", "thermitecharge", 1)
                exports['np-thermite']:OpenThermiteGame(function(success)
                    if success then
                        TriggerServerEvent('np-doors:change-lock-state', 272, false) 
                        TriggerServerEvent("np-doors:change-lock-state", 273, false)
                        exports["np-dispatch"]:dispatchadd('10-90', "Bobcat Security Robbery", "500")
...
AddEventHandler('np-heists:bobcat_armed_door', function()
    ...
    if pd >= 0 then
        if Dist < 1.5 then
            ...
            TriggerEvent('np-heists:bobcat_explosion_ped')
            TriggerServerEvent('np-heists:spawnpeds')
...
RegisterNetEvent('blow_door', function()
    TriggerServerEvent('np-heists:bobcatLog')
    TriggerServerEvent('np-bobcat:blow_door')
end)

RegisterNetEvent('np-heists:bobcat_explosion_ped')
AddEventHandler('np-heists:bobcat_explosion_ped', function()
...
RegisterNetEvent('np-heists:explosion_ped_walk') -- PED WALK
AddEventHandler('np-heists:explosion_ped_walk', function()
    exports["np-dispatch"]:dispatchadd('10-90', "Bobcat Security Robbery", "500")
...
exports["np-polytarget"]:AddCircleZone("np_bobcat_loot_1",  vector3(886.61, -2287.32, 29.92), 1.0, {
    useZ = true,
})

exports["np-polytarget"]:AddCircleZone("np_bobcat_loot_3",  vector3(881.4, -2282.62, 30.42), 1.0, {
    useZ = true,
})

exports["np-polytarget"]:AddCircleZone("np_bobcat_loot_2",  vector3(882.49, -2286.17, 30.37), 1.0, {
    useZ = true,
})

exports["np-interact"]:AddPeekEntryByPolyTarget('np_bobcat_loot_1', {{
    id = "np_bobcat_loot_1",
...
exports["np-interact"]:AddPeekEntryByPolyTarget('np_bobcat_loot_2', {{
    id = "np_bobcat_loot_2",
...
exports["np-interact"]:AddPeekEntryByPolyTarget('np_bobcat_loot_3', {{
    id = "np_bobcat_loot_3",
