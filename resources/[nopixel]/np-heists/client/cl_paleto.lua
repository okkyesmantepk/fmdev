...
AddEventHandler('np-paleto:box', function()
    ...
    if distance < 1.5 then
        if exports["np-inventory"]:hasEnoughOfItem("thermitecharge", 1) then
            ...
            PaletoStep1()
            exports["np-dispatch"]:dispatchadd('10-90', "Paleto Bank Robbery", "108")
...
RegisterNetEvent('np:createpaletomoney')
AddEventHandler('np:createpaletomoney', function(toggle)
    if toggle == true then
...
function CreateMoney()
    ...
    TriggerEvent('np:CleanClientVaultD')
    TriggerEvent("np:createpaletomoney" ,true)
    Citizen.Trace("Crates Spawned :  Sucess")
    Citizen.Wait(240000)
    Citizen.Wait(60000)
    TriggerEvent("np:createpaletomoney" ,false)
end
