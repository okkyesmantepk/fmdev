RegisterNetEvent('np-heists:bay_city:first_door')
AddEventHandler('np-heists:bay_city:first_door', function()
    ...
    if pd >= 4 then
        if distance < 1.5 then
            if exports["np-inventory"]:hasEnoughOfItem("thermitecharge", 1) then
                firstDoorBC()
                TriggerServerEvent('np-heists:bayCityLog')
                exports["np-dispatch"]:dispatchadd('10-90', "Bay City Bank", "500")
                ...
                exports['np-thermite']:OpenThermiteGame(function(success)
                    if success then
                        ...
                        TriggerServerEvent('np-doors:change-lock-state', 563, false) 
                        Citizen.Wait(3000)
...
RegisterNetEvent('np-heists:bay_city:second_door')
AddEventHandler('np-heists:bay_city:second_door', function()
    local pd = exports["isPed"]:isPed("countpolice")
    if pd >= 4 then
        exports["np-dispatch"]:dispatchadd('10-90', "Bay City Bank", "500")
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['np-taskbar']:taskBar(15000, 'Opening Door.')
        if finished == 100 then
            TriggerEvent('ghost:createTrolleys')
            TriggerServerEvent('pStopSecondDoor')
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerServerEvent('np-doors:change-lock-state', 564, false) 
            -- Create Trolleys.
        end
    end
end)

RegisterNetEvent('np-heists:bay_city:second_door:sv')
AddEventHandler('np-heists:bay_city:second_door:sv', function()
    TriggerServerEvent('np-heists:second_door:cl')
end)

exports["np-polytarget"]:AddCircleZone("np_baycity_unlock", vector3(-1309.13, -815.79, 17.7), 0.45, {
    useZ = true
})

exports["np-interact"]:AddPeekEntryByPolyTarget("np_baycity_unlock", {{
    event = "np-heists:bay_city:second_door:sv",
...
AddEventHandler("np-inventory:itemUsed", function(item)
    if item == "thermitecharge" then
        TriggerServerEvent('np-heists:first_door:cl')
    end
end)