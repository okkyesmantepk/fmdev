...
RegisterNetEvent("np-housing:enterBUILDINGCMON", function()
    InteractionPressed()
end)
...
RegisterCommand("np-housing:getGarages", function()
...
function getIsPropLock()
    ...
    local pp =  RPC.execute("np-housing:isHousingLockedServer", _propertyID)
...
AddEventHandler('np-housing:phone:check', function(type)
    local isComplete, propertyId, dist, zone = Housing.func.findClosestProperty()
    if not isComplete then
    ...
    local isThisPropertyOwned = RPC.execute("np-housing:isThisPropertyOwned", propertyId)
    ...
    if type == "edit" then
        ...
        TriggerEvent('np-phone:editingHouse',id,street,price)
    else
        TriggerEvent('np-phone:sentHouse',nil,id,street,price,category)
...
RegisterCommand('rent', function()
    TriggerEvent('np-housing:rent')
end)

RegisterNetEvent("np-housing:rent", function()
    local isComplete, propertyId, dist, zone = Housing.func.findClosestProperty()
    if not isComplete then
        TriggerEvent("DoLongHudText", propertyId, 2)
        return
...
RegisterNetEvent("np-housing:rent2", function(pParams)
...
RegisterNetEvent("np-housing:buyed", function(pParams)
    print('hello?')
...
RegisterNetEvent("np-housing:edit", function()
end)

RegisterNetEvent("np-housing:editOptions", function(pParams)
...
RegisterNetEvent('np-housing:logShit')
AddEventHandler('np-housing:logShit', function(hID, Price)
    TriggerServerEvent('np-housing:buyLog', hID, Price)
end)