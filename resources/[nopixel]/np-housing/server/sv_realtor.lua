...
GetUserByCid = function(cid)
    for index, value in pairs(GetPlayers()) do
        local xPlayer = exports['np-base']:getModule("Player"):GetUser(tonumber(value))
...
AddEventHandler("np-housing:sell", function(pCid, pPropertyId)
    ...
    print('here')
    TriggerClientEvent("np-housing:buy", pCid, pPropertyId, seller)
end)
...
RPC.register("np-housing:buy", function(src, pPropertyId, pTotal, pTax, pSeller)
    ...
    TriggerClientEvent("np-housing:loadHousingClient",src)
    TriggerClientEvent('np-housing:logShit', src, pPropertyId, pTotal)
...
RPC.register("np-housing:isHousingLockedServer", function(pSrc, pHid)
    print(pHid)
...
RegisterNetEvent("np-housing:getGarages", function()
    local src = source
    ...
    for i, v in ipairs(houses) do
        ...
        TriggerClientEvent("menu:send:np-housing:garages", src,json.decode(garagedata),v.hid)
    end
end)