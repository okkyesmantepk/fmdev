...
RPC.register("np-housing:isThisPropertyOwned", function(src, propertyId)
    local result = MySQL.query.await([[
...
RPC.register("np-phone:giveKey", function(src, pHouseId, pPlayerId)
    ...
    local user2 = exports['np-base']:getModule("Player"):GetUser(pPlayerId)
    ...
    TriggerClientEvent("np-phone:notification", pPlayerId, "fas fa-key", "Keys", "Você recebeu a chave da propriedade " .. Housing.info[pHouseId]["street"], 5000)
...
RegisterServerEvent('np-housing:buyLog')
AddEventHandler('np-housing:buyLog', function(pHouse, price)
    local src = source
    local user = exports["np-base"]:getModule("Player"):GetUser(src)
