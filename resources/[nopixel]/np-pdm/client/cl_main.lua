...
AddEventHandler("np-pdm:testdrive", function()
	if exports["np-business"]:IsEmployedAt("pdm") or #(vector3(-30.31887, -1089.345, 26.04948) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
AddEventHandler("np-pdm:EnableBuy", function()
	if #(vector3(-30.31887, -1089.345, 26.04948) - GetEntityCoords(PlayerPedId())) > 50.0 and exports["np-business"]:IsEmployedAt("pdm") then
		return
	end
...
function DrawPrices()
	for i = 1, #carspawns do
		if #(vector3(carspawns[i]["x"],carspawns[i]["y"],carspawns[i]["z"]) - GetEntityCoords(PlayerPedId())) < 1.5 then
			...
			if exports["np-business"]:IsEmployedAt("pdm") then
...
function CloseCreator(name, veh, price, plate)
	Citizen.CreateThread(function()
		...
		if not boughtcar then
			...
			TriggerServerEvent('np-pdm:BuyVehicle', plate, vehname, vehicle, price, VehicleProps)
			TriggerServerEvent('np-cardealer:log', exports['isPed']:isPed('cid'), vehname, price, plate, 'PDM')
			TriggerServerEvent('np-financials:business_money', price / 10, 'pdm', 'add')
			exports['np-interface']:hideInteraction()
			DespawnSaleVehicles()
			SpawnSaleVehicles()
		end
	...
	while true do
		...
		if vehshop.opened then
			...
			for i,button in pairs(menu.buttons) do
				...
				if exports["np-business"]:IsEmployedAt("pdm") and i >= vehshop.menu.from and i <= vehshop.menu.to then
...
AddEventHandler('pdmvehshop:spawnVehicle', function(v)
	...
	if playerPed and playerPed ~= -1 then
		...
		exports['np-vehicles']:SetVehicleIdentifier(veh, netid)
	end
end)
...
RegisterNetEvent('np-fuel:pdmmenushit', function()
...
RegisterInterfaceCallback('np-pdm:pdmcom', function(data, cb, pParams)
	cb({ data = {}, meta = { ok = true, message = '' } })
	exports['np-interface']:openApplication('textbox', {
		callbackUrl = 'np-pdm:send_pdm',
		key = 1,
		items = {
		  {
			icon = "address-card", 
...
RegisterInterfaceCallback('np-pdm:send_pdm', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
	TriggerServerEvent("np-pdm:wtf", data.values.pPDMPayCom)
end) 



RegisterNetEvent("np-pdm:commissionpdm") 
AddEventHandler("np-pdm:commissionpdm", function(args)
	print("YO")
	if exports["np-business"]:IsEmployedAt("pdm") then
		print("IM PDM WORKER")
		local amount = args
...
RegisterNetEvent("np-pdm:testdrivepdm")
AddEventHandler("np-pdm:testdrivepdm", function()
	if exports["np-business"]:IsEmployedAt("pdm") then
		TriggerEvent('np-pdm:testdrive')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
...
RegisterCommand('enablebuy', function(source,args)
	TriggerEvent('np-pdm:enableBuypdm')
end)

RegisterNetEvent("np-pdm:enableBuypdm")
AddEventHandler("np-pdm:enableBuypdm", function()
	if exports["np-business"]:IsEmployedAt("pdm") then
		TriggerEvent('np-pdm:EnableBuy')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
