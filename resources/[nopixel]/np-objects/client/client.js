;(function () {
  ...
  !(function () {
    ...
    setImmediate(async () => {
      ...
      Number(objectTable.length) === 0 && emitNet('np-objects:requestObjects')
    })
    onNet('np-objects:loadObjects', async (pObjectData) => {
      console.log('[np-objects] Load Objects')
    ...
    on('np-polyzone:enter', async (name, data) => {
      if (name !== 'object_zone') {
        return
      }
      ...
      console.log('[np-objects] Entered zone, load objects in distance.')
    ...
    on('np-polyzone:exit', (name, data) => {
      ...
      console.log('[np-objects] Left zone, unload objects in distance.')
    ...
    onNet('np-objects:clearObjects', (objectData) => {
    ...
    onNet('np-objects:updateObjects', (objectData) => {
      Object.entries(objectData).forEach(([k, v]) => {
        let findData = newObjTable.findIndex(
          (fountData) => fountData.id.toString() === v.toString()
        )
    ...
    async function prepareObject(objectData) {
      console.log('[np-objects] Adding object: ', objectData)
      let coords = {
    ...
    function DelObject(obj) {
      let deleted = RPC.execute('np-objects:DeleteObject', obj)
      return deleted
    }
    exports('DelObject', DelObject)
    function UpdateObject(object, updatedObject) {
      RPC.execute('np-objects:UpdateObject', object, updatedObject)
    }
    ...
    RegisterCommand(
      ...
      (src, args, user) => {
        ...
        console.log('[np-objects] getObject return: ', consoleObj)
      },
      false
    )
    RegisterCommand(
  ...
  pMetaData
      ...
      return await RPC.execute('np-objects:SaveObject',pRando,pObjectModel,pCoords.coords,pCoords.rotation,pHeading,pMetaData)
    }
