...
RegisterInterfaceCallback("np-signin:handler", function(data, cb)
...
RegisterInterfaceCallback("np-signin:spawnVehicle", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  local eventData = data.key
...
AddEventHandler("np-signin:peekAction", function(pArgs, pEntity, pContext)
  ...
  print('[np-signin:peekAction] MenuData:' ..' ' ,name)
  exports["np-interface"]:showContextMenu(MenuData[name])
end)
