...
RPC.register("np-phone:addToCurrentBanks", function(pSource, pBank)
...
RPC.register("np-phone:getCurrentBanks", function(pSource)
    return currentBanks
end)

RPC.register("np-phone:requestPlayerToGroup", function(pSource, pCid, pGroupID)
...
RPC.register("np-phone:addPlayerToGroup", function(pSource, pCid, pGroupID, pSenderID)
...
RPC.register('np-phone:removePlayerFromGroup', function(pSource, pCid, pGroupID)
...
RPC.register("np-phone:createGroup", function(pSource)
...
RPC.register('np-phone:deleteGroup', function(pSource, pGroupID)
    local jsonDecodedGroups = json.decode(groupTable)
    table.remove(jsonDecodedGroups, pGroupID)
    groupTable = json.encode(jsonDecodedGroups)
end)

RPC.register('np-phone:getMyGroup', function(pSource, pCid)
    local jsonDecodedGroups = json.decode(groupTable)
    for k,v in pairs(jsonDecodedGroups) do
        local jsonDecodedGroupMembers = json.decode(v.members)
        for j,s in pairs(jsonDecodedGroupMembers) do
