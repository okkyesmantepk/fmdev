...
local documents = {}
RPC.register('np-phone:getDocuments', function(pSource, pType)
...
RPC.register("np-phone:sendDocumentEvent", function(pSource, pTarget, pDocumentID)
    TriggerClientEvent("np-phone:sendDocumentEvent", pTarget, pDocumentID)
end)

RegisterNetEvent("np-phone:checkForNonDocumentedLicenses", function(pCid)
    local sex = "sex"
...
RPC.register("np-phone:shareDocumentPermanent", function(pSource, dID, tCID)
...
RPC.register("np-phone:acceptDocumentReceiveRequest", function(pSource, pDocumentID)
    local doc = MySQL.query.await([[
        SELECT * FROM __documents WHERE id = ?
    ]], {pDocumentID})
...
RPC.register('np-phone:deleteDocument', function(pSource, pId)
    exports.oxmysql:execute("DELETE FROM __documents WHERE id = ?",{pId})
end)

RPC.register('np-phone:updateDocument', function(pSource, pId, pTitle, pText)
...
RPC.register("np-phone:createDocument", function(pSource, pOwner, pTitle, pText, pType)
    MySQL.query.await([[
