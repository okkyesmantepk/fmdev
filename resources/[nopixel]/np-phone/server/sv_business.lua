...
RPC.register("np-phone:sendChargeCustomerNotif", function(pSource, BusinessName, GroupID, stateID, amount, reason)
    local tSrc = getPlayerFromCidForCharge(tonumber(stateID))
    local tax = exports['np-business']:taxTotal(amount)
    ...
    TriggerClientEvent("np-phone:chargeCustomerNotif", tSrc, { 
...
RPC.register("np-phone:chargeCustomerBus", function(pSource, amount, GroupID, stateID, sID)
    local amtTax = amount
    local tSrc = getPlayerFromCidForCharge(tonumber(stateID))
    local user = exports['np-base']:getModule("Player"):GetUser(tSrc)
    local char = user:getCurrentCharacter()
    ...
    if bank >= tonumber(amtTax) then
        ...
        TriggerClientEvent("np-phone:businessRegisterNoti", sID, "SERVICE CHARGE", "They couldn't accept the payment")
        TriggerClientEvent("np-phone:businessRegisterNoti", tSrc, "SERVICE CHARGE", "Not enough money to complete the payment.")
        return
    end
    TriggerClientEvent("np-phone:businessRegisterNoti", tSrc, "SERVICE CHARGE", "Payment Successful.")
    TriggerClientEvent("np-phone:businessRegisterNoti", tSrc, "SERVICE CHARGE", "You have been deducted " .. amtTax .. " from you're bank account")
    TriggerClientEvent("np-phone:businessRegisterNoti", sID, "SERVICE CHARGE", "They accepted the payment")
    return
end)
...
RPC.register("np:getBusinessLogs", function(pSource, pBusiness)
...
RPC.register("np-business:payExternal", function(pSource, pCid, pAmount, pBusinessID)
