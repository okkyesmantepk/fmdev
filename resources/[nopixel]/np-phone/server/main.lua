...
RPC.register("np-phone:getPhoneNumber", function(pSource)
...
RPC.register("np-phone:sendThePM", function(src, msg)
...
RPC.register("acceptBusinessCharge", function(pSource, pBID, pAmt, pCID)
    ...
    local user = exports['np-base']:getModule("Player"):GetUser(SID)
    local char = user:getCurrentCharacter()
...
RegisterNetEvent("np-phone:h-Lock:sv", function(hid, status)
    print("lock house ? " ..hid .. " " ..status)
    exports.oxmysql:execute("UPDATE housing SET status = ? WHERE hid = ?",{status,hid})
end)

RPC.register("np-financials:getCurrentBank", function(pSource)
    local user = exports['np-base']:getModule("Player"):GetUser(pSource)
    local char = user:getCurrentCharacter()
    local cid = char.id
...
RPC.register("np-apartments:getCurrentApartment", function(src)
...
RPC.register("np-housing:getKeyHolderz", function(src,hid)
    print(src,hid)
    local myKeyz = {}
...
RPC.register("np-housing:removeKey:phone", function(src,cid,hid)
    local keys = MySQL.query.await([[
...
RPC.register('np-phone:getHousePrice', function(source,hid)
    ...
    local user = exports["np-base"]:getModule("Player"):GetUser(src)
    local hIds = hid
    local houseInfo = exports['np-housing']:getCurrentOwned(src)
...
RPC.register("np-phone:addDOJ", function(pSource, pJob)
    local user = exports['np-base']:getModule("Player"):GetUser(pSource)
    local char = user:getCurrentCharacter()
    local cid = char.id
...
RPC.register("np-phone:removeDOJ", function(pSource, pJob)
    local user = exports['np-base']:getModule("Player"):GetUser(pSource)
    local char = user:getCurrentCharacter()
    local cid = char.id
...
RPC.register("np-phone:getDOJ", function(src)
    return availableDOJ
end)

local availableDrivers = {}

RPC.register("np-phone:addAbdulTaxi", function(src)
    local user = exports['np-base']:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local cid = char.id
    local phonenumber = char.phone_number
...
RPC.register("np-phone:getAbdulTaxies", function(src)
    return availableDrivers
end)

RegisterNetEvent("np-phone:getAbdulTaxies", function()
    TriggerClientEvent("np-phone:manualUpdateAbdulTaxies", source)
end)

RPC.register("np-phone:removeAbdulTaxi", function(src)
    local user = exports['np-base']:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local cid = char.id
...
AddEventHandler('np-phone:purchasedCar', function(plate,amount,sellerID)
    local src = source
    local user = exports["np-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local player = exports["np-base"]:getModule("Player"):GetUser(sellerID)
