...
RegisterNUICallback('racing:event:start', function(data)
  ...
  if canJoinOrErr ~= true then
    exports["np-phone"]:phoneNotification("racen",canJoinOrErr,"RACING", 3000)
    return
  end
...
RegisterNetEvent("np-racing:removedActiveRaced")
AddEventHandler("np-racing:removedActiveRaced", function(id)
    openRacingApp()
...
RegisterNUICallback('np-phone:racing:setGPS', function(data)
  local mapID = data.mID
  exports["np-racing"]:locateRace(mapID)
  phoneNotification("racen","GPS Set to your map.","RACING")
end)

RegisterNUICallback('np-phone:racing:seePreview', function(data)
  local mapID = data.mID
  if not mapID then return end
  id = tostring(mapID)
  exports["np-racing"]:previewRace(mapID)
...
RegisterNUICallback('np-phone:r-users', function(data, cb)
  ...
  if race ~= nil then
    ...
    local racer = RPC.execute('np-phone:racing:getRacer', raceID)
    cb(racer)
  end
...
RegisterNetEvent('np-racing:pendingRacenotif')
AddEventHandler("np-racing:pendingRacenotif", function(race)
  ...
  exports["np-phone"]:phoneNotification("racen","Pending race available...","From the PM", 12000)
end)
-- Citizen.CreateThread(function()
--   while true do
--     Wait(5000)
