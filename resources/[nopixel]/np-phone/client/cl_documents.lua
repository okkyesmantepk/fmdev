RegisterNUICallback("getDocuments", function(data, cb)
    SendNUIMessage({
        ...
        documentData = RPC.execute("np-phone:getDocuments", data.pType)
    })
end)

RegisterNUICallback("deleteDocument", function(data, cb)
    RPC.execute("np-phone:deleteDocument", data.documentID)
end)

RegisterNUICallback("updateDocument", function(data, cb)
    RPC.execute("np-phone:updateDocument", data.documentID, data.documentTitle, data.documentText)
end)

RegisterNUICallback("createDocument", function(data, cb)
    RPC.execute("np-phone:createDocument", exports['isPed']:isPed("cid"), "New Document", "Text Goes Here", data.documentType)
end)

RegisterNUICallback("shareDocumentPermanent", function(data)
    RPC.execute("np-phone:shareDocumentPermanent", data.documentID, data.targetCID)
end)
...
RegisterNUICallback("shareDocumentLocal", function(data)
    ...
    for k, v in pairs(players) do
        RPC.execute("np-phone:sendDocumentEvent", v, data.documentID)
    end
end)


RegisterNetEvent("np-phone:sendDocumentEvent", function(documentID)
...
RegisterNUICallback("acceptDocumentReceiveRequest", function(data)
    local pDocShared = RPC.execute("np-phone:acceptDocumentReceiveRequest", data.documentID)
    SendNUIMessage({
