...
RegisterCommand('renten', function()
  TriggerEvent('np-housing:rent_activate_cl')
end)
...
RegisterNUICallback('sendThePM', function(data, cb)
  ...
  RPC.execute("np-phone:sendThePM", thePM)
end)

RegisterCommand("fuckmeintheasdasvf", function()
  RPC.execute("np-phone:sendThePM", 'f')
end)
...
RegisterNetEvent("np-phone:setSomeShit", function(bg, model, twatNotify, callNotify)
  SetResourceKvp(exports['isPed']:isPed('cid').."-bg", bg)
  SetResourceKvpInt(exports['isPed']:isPed('cid').."-pmodel", model)
  SetResourceKvpInt(exports['isPed']:isPed('cid').."-ntwat", twatNotify)
  SetResourceKvpInt(exports['isPed']:isPed('cid').."-ncall", callNotify)
...
RegisterNUICallback('btnKeyHolders', function(data, cb)
  local myKittens = {}
  local keys = RPC.execute('np-housing:getKeyHolderz', data.hid)
  SendNUIMessage({
...
RegisterNUICallback('whatthefuckdoesthisshitwork', function(data,cb)
  local cid = data.cid
  local hid = data.hid
  local success,msg = RPC.execute('np-housing:removeKey:phone',cid, hid)
  if success then
...
RegisterNUICallback('OpenCamera', function(data,cb)
  RPC.execute('np-phone:openCamera', data.cameraId)
end)
...
RegisterInterfaceCallback("np-phone:signInHandler", function(data, cb)
  if data.key.sign_in then
    RPC.execute("np-phone:addDOJ", data.key.job)
  else
    RPC.execute("np-phone:removeDOJ", data.key.job)
  end
...
RegisterNUICallback('getActiveDOJ', function(data,cb)
      SendNUIMessage({
        ...
        penisinmybuttholefuckshit = RPC.execute("np-phone:getDOJ")
      })
end)
...
RegisterNUICallback('abdulSignIn', function(data,cb)
    ...
    if taxiStatus then
      RPC.execute("np-phone:addAbdulTaxi")
      addAbdulTaxi()
    ...
    elseif not taxiStatus then
      RPC.execute("np-phone:removeAbdulTaxi")
      removeAbdulTaxi()
      local availableDrivers = getAbdulTaxi()
      SendNUIMessage({
        openSection = "addTaxi",
...
RegisterNetEvent("np-phone:manualUpdateAbdulTaxies", function(availableDrivers)
  addAbdulTaxi()
  local availableDrivers = getAbdulTaxi()
  SendNUIMessage({
    openSection = "addTaxi",
...
function addAbdulTaxi () 
  currenttable = {}
  local table = RPC.execute("np-phone:getAbdulTaxies")
--  local currenttable = {}
...
RegisterNUICallback('purchaseCar', function(data)
  TriggerServerEvent('np-phone:purchasedCar', sellCarPlate, data.amount,data.senderID)
end)
...
RegisterNUICallback('unlockHouses', function(data)
  ...
  TriggerServerEvent('np-phone:house:status',hId,statuus)
  Wait(200)
  TriggerEvent('updateUIHousing')
end)

RegisterNetEvent("np-phone:h-Lock", function(hId, status)
  TriggerServerEvent("np-phone:h-Lock:sv", hId, status)
...
RegisterNUICallback('editHousingOwn', function()
  -- if exports['np-housing']:isNearProperty() then
    if exports["np-housing"]:canEdit() then
      local isComplete, _propertyID, dist, zone = exports['np-housing']:findClosetedPerson()
      TriggerEvent("np-housing:editOptions",{action = "enter", propertyId = _propertyID})
    else
      SendNUIMessage({
...
function openGui(isBurner,cid,pNumber)
   TriggerEvent("np-spawn:phonerefresh") --Refreshes jobs
  ...
  if hasPhone() then
    ...
    SendNUIMessage({
      ...
      hasDevice = device, 
      hasDecrypt = decrypt, 
      pNum = RPC.execute("np-phone:getPhoneNumber") or pNumber, -- Do later 
...
RegisterNetEvent("phone:sendThePM", function(name, msg)
  if exports["np-inventory"]:hasEnoughOfItem("thepmdongle",1,false,true) then
...
RegisterCommand('house', function()
  local isComplete, propertyID, dist, zone = exports['np-housing']:findClosetedPerson()
...
RegisterNUICallback('leaveEditMode', function()
  TriggerEvent('np-housing:editOptions', { action = "exit" })
end)
...
RegisterNUICallback('openFurnitureUI', function()
  ...
  TriggerEvent('np-housing:editOptions', { action = "furniture" })
end)

RegisterNUICallback('setBackdoorUI', function()
  TriggerEvent('np-housing:editOptions', { action = "backdoor" })
end)

RegisterNUICallback('setWardrobeUI', function()
  TriggerEvent('np-housing:editOptions', { action = "wardrobe" })
end)

RegisterNUICallback('setCraftingUI', function()
  TriggerEvent('np-housing:editOptions', { action = "crafting" })
end)

RegisterNUICallback('setGarageUI', function()
  TriggerEvent('np-housing:editOptions', { action = "garage" })
end)

RegisterNUICallback('setStashUI', function()
  TriggerEvent('np-housing:editOptions', { action = "stash" })
end)
...
RegisterNUICallback('purchaseHouse', function(data)
  ...
  if rentEnabled then
   --   TriggerEvent('np-phone:sentHouse')
      local success, msg = RPC.execute("np-housing:buy", data.hid, data.price, 0, exports['isPed']:isPed("fullname"))
      TriggerEvent("DoLongHudText",msg,1)
     return
  end
...
RegisterInterfaceCallback("np-phone:SellRentEnable", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterNetEvent('np-phone:signInInfoTowing')
AddEventHandler('np-phone:signInInfoTowing', function()
  phoneNotification("email", "Grab the towtruck parked in the bay.", "Job Center")
end)

RegisterNetEvent('np-phone:towing_offer')
AddEventHandler('np-phone:towing_offer', function()
...
RegisterNUICallback('accept_tow_job', function(data)
  TriggerEvent('np-jobs:towing_accept')
end)

-- 24/7 Delivery --

RegisterNetEvent('np-phone:signInInfoDelivery')
AddEventHandler('np-phone:signInInfoDelivery', function()
  phoneNotification("email", "Signed in as 24/7 Delivery Driver.", "Job Center")
end)

RegisterNetEvent('np-phone:delivery_offer')
AddEventHandler('np-phone:delivery_offer', function()
...
RegisterNUICallback('accept_delivery_job', function(data)
    TriggerEvent('np-jobs:deliveries:getCar')
end)

-- Dodo Logistics --

RegisterNetEvent('np-phone:signInDodo')
AddEventHandler('np-phone:signInDodo', function()
  phoneNotification("email", "Signed in as a Dodo Logistics worker.", "Job Center")
end)

RegisterNetEvent('np-phone:dodoJobOffer')
AddEventHandler('np-phone:dodoJobOffer', function()
...
RegisterNUICallback('accept_dodo_job', function(data)
    TriggerEvent('np-jobs:spawn-truck')
end)

RegisterNetEvent('np-phone:goToLocation')
AddEventHandler('np-phone:goToLocation', function()
  phoneNotification("email", "Go to the delivery location on the GPS.", "Job Center")
end)

RegisterNetEvent('np-phone:goBackDepo')
AddEventHandler('np-phone:goBackDepo', function()
  phoneNotification("email", "Return back to the depo.", "Job Center")
end)

-- House Robberies --

RegisterNetEvent('np-phone:signInInfoHouseRobbery')
AddEventHandler('np-phone:signInInfoHouseRobbery', function()
  phoneNotification("email", "Signed in as Criminal.", "Job Center")
end)

RegisterNetEvent('np-phone:house_rob_offer')
AddEventHandler('np-phone:house_rob_offer', function()
...
RegisterNUICallback('accept_house_robbery', function(data)
    TriggerEvent('np-activities:acceptJob')
end)

-- Chopshop --

RegisterNetEvent('np-phone:signInInfoChopshop')
AddEventHandler('np-phone:signInInfoChopshop', function()
  phoneNotification("email", "Signed in as Chopshop.", "Job Center")
end)

RegisterNetEvent('np-phone:chop_shop_offer')
AddEventHandler('np-phone:chop_shop_offer', function()
...
RegisterNUICallback('accept_chop_shop', function(data)
    TriggerEvent('np-chopshop:acceptOffer')
end)

-- Oxy Runs --
RegisterNetEvent('np-phone:oxyRunNoti')
AddEventHandler('np-phone:oxyRunNoti', function(pText)
  phoneNotification("email", pText, "Job Center")
end)

RegisterNetEvent('np-phone:oxyrun:offer')
AddEventHandler('np-phone:oxyrun:offer', function()
...
RegisterNUICallback('accept_oxy_run', function(data)
    TriggerEvent('np-oxy:Accept:Offer')
end)

-- Paycheck --

RegisterNetEvent('np-phone:paymentNoti')
AddEventHandler('np-phone:paymentNoti', function(pAmt, pType)
  phoneNotification("email", "$"..pAmt..".00 has been added to your "..pType.. " Balance", "Job Center")
end)

RegisterNetEvent('np-phone:businessRegisterNoti')
AddEventHandler('np-phone:businessRegisterNoti', function(pBusinessName, pText)
  phoneNotification("charge", pText, pBusinessName)
end)

-- Fishing --

RegisterNetEvent('np-phone:signInFishing')
AddEventHandler('np-phone:signInFishing', function()
