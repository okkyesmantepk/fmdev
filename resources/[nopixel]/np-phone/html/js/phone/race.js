...
//    console.log("PENDING ENTRIES",pendingRaces.parent().has('.component-paper').length)
   ...
   $('.pGPS').click(function(){
      ...
      $.post('http://np-phone/np-phone:racing:setGPS', JSON.stringify({
         mID: mID,
     }));
   })
   $('.rPreview').click(function(){
      ...
      $.post('http://np-phone/np-phone:racing:seePreview', JSON.stringify({
         mID: mID,
     }));
   })
...
$('.pending-entries').on('click', '.pJoin',function(){
   ...
   $.post('http://np-phone/racing:event:join', JSON.stringify({
      id: eID
   }));
});

$('.pending-entries, .active-entries').on('click', '.pLeave', function(){
   $.post('http://np-phone/racing:event:leave', JSON.stringify({}));
});
...
$('.pending-entries, .active-entries').on('click', '.pEndRace',function(){
   $.post('http://np-phone/racing:event:end', JSON.stringify({}));
});
