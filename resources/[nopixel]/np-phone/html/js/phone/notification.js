...
btnGasAccept = () => {
	...
	if (gasType == "gas") { 
		$.post("https://np-phone/purchase_gas", JSON.stringify({}))
	} else if (gasType == "charge") {
		$.post("https://np-phone/acceptBusinessCharge", JSON.stringify({
			pAmount: chargeAMT,
			pBID: companyID,
			pTargetID: targetID,
			pSenderID: senderIDX
		...
		$.post("https://np-phone/purchaseCar", JSON.stringify({
			//plate: plate,
			amount: amount,
			senderID: sellerIDc,
		}))
        ...
        $.post("https://np-phone/accept_tow_job", JSON.stringify({}))
    } else if (gasType == "jobofferdelivery") {
        $.post("https://np-phone/accept_delivery_job", JSON.stringify({}))
    } else if (gasType == "jobofferdodo") {
        $.post("https://np-phone/accept_dodo_job", JSON.stringify({}))
    } else if (gasType == "house_robbery") {
        $.post("https://np-phone/accept_house_robbery", JSON.stringify({}))
    } else if (gasType == "jobofferoxy") {
        $.post("https://np-phone/accept_oxy_run", JSON.stringify({}))
	} else if (gasType == "jobofferchop") {
        $.post("https://np-phone/accept_chop_shop", JSON.stringify({}))
	} else if (gasType == "documents") {
		$.post("https://np-phone/acceptDocumentReceiveRequest", JSON.stringify({
		...
		$.post("https://np-phone/onPlayerJoinBankBusters", JSON.stringify({
...
btnJobAccept = () => {
	...
	$.post("https://np-phone/jobOfferAccepted", JSON.stringify({
	}))
}
