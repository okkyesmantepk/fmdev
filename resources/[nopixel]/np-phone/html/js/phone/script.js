        ...
        if (action !== undefined) {
            switch (action) {
                ...
                case "getCallHistory":
                    if (callStates[currentCallState] === "isCallInProgress" && currentContainer !== "incoming-call")
                        ...
                        if (!isBurnah) {
                            ...
                            $.post('http://np-phone/getRecentCallsByCid', JSON.stringify({
                                cid: burnahCid
                            }));
                        }
                    break;
                ...
                case "doj-app":
                    ...
                    $.post('https://np-phone/getActiveDOJ', JSON.stringify({}))
                    break;
                case "camera-app":
                    $(".camera-inside").empty();
                    openContainer('camera')
                    $.post('https://np-phone/getCurrentCameras', JSON.stringify({}))
                    break;
                case "lsbn":
                    openContainer('lsbn');
                    break;
                ...
                case "btnBankBusters":
                    ...
                    $.post('https://np-phone/getCurrentBanks', JSON.stringify({}))
                    setTimeout(() => {
                        $.post('https://np-phone/inBankBusterGroup', JSON.stringify({}))
                    }, 1500)
//                    $.post('https://np-phone/getCurrentBanks', JSON.stringify({}))
                 //   $.post('https://np-phone/sendNotifAlert', JSON.stringify({
                 //       type: "alert"
                 //   }))
                    break;
                case "documents":
                    ...
                    $.post('https://np-phone/getDocuments', JSON.stringify({
                        pType: txt
                    }))
                    break;
                case "diamond":
                    ...
                    $.post('https://np-phone/getDiamondSportsEvents', JSON.stringify({}))
                    break;
                default:
                    $.post('http://np-phone/' + action, JSON.stringify({}));
                    break;
    ...
    $(".document-submit-cid").click(function (e) {
        ...
        setTimeout(() => {
            ...
            $.post('https://np-phone/shareDocumentPermanent', JSON.stringify({
                documentID: pDocID,
                targetCID: ptCID
            }))        
        })
    ...
    $('.btn-share-local').click(function() {
        ...
        setTimeout(() => {
            let pDocID = document.getElementById("document-id").value;
            $.post('https://np-phone/shareDocumentLocal', JSON.stringify({
                documentID: pDocID,
            }))        
        })
    })
    ...
    $('.btn-drop-qr-code').click(function() {
        ...
        setTimeout(() => {
            let pDocID = document.getElementById("document-id").value;
            $.post('https://np-phone/dropQRCode', JSON.stringify({
                documentID: pDocID,
            }))        
        })
    })
    ...
    $('.btn-delete-document').click(function(){
        ...
        setTimeout(() => {
            ...
            $.post('https://np-phone/deleteDocument', JSON.stringify({
...
function createDiamond(events, isEmployed) {
    $.each(events, function (__, event) {
        ...
        $('#delete-diamondEvent').click(function(){
            ...
            $.post('http://np-phone/deleteDiamondEvent', JSON.stringify({
...
function createDOJDepartments(doj) {
    ...
    window.addEventListener('message', function (event) {
        ...
        if (item.openPhone === true) {
            ...
            $.post('http://np-phone/retreivePhoneContacts', JSON.stringify({}));
...
function burnahMessages() {
    $.post('https://np-phone/burnahMessages', JSON.stringify({
        cid: burnahCid
    }))
}
...
function createCamera(cameras) {
    ...
    $(".eye-camera").click(function () {
        ...
        $.post('https://np-phone/OpenCamera', JSON.stringify({
            cameraId: cameraId
        }));
    })
    ...
    $(".remove-camera").click(function() {
        ...
        $(".removeCamera").empty();
        $.post("https://np-phone/getCameraAccessPlayers", JSON.stringify({
            cameraId: cameraId
        }))
    })
};
...
$("#add-camera-submit").click(function() {
    ...
    complateInputQuick()
    setTimeout(() => {
        $.post("https://np-phone/addUserToCamera", JSON.stringify({
            cameraId: camid,
...
$("#remove-camera-submit").click(function() {
    ...
    setTimeout(() => {
        $("#cameraRemoveID").val("");
        $.post("https://np-phone/removeUserFromCamera", JSON.stringify({
            cameraId: camid,
            targetPhoneNumber: cid
        }))
    }, 1500)
...
function gpsJobCenter(data) {
    $.post(
      'https://np-phone/jobcenter-setgps',
...
$('.diamondsports-createevent').on('click', '#submit-input', function(){ 
    ...
    $.post('http://np-phone/submitNewEventDiamondSports', JSON.stringify({
...
$('#submit-input-the-pm').on('click', function() {
    ...
    setTimeout(() => {
        $('#the-pm-input').val("")
        $.post("https://np-phone/sendThePM", JSON.stringify({
            thePM: misinput
        }))
    }, 1900)
})
        ...
        case "messages":
            ...
            setTimeout(function () {
                if (!isBurnah) {
                    ...
                    $.post('http://np-phone/getTextsByCid', JSON.stringify({
                        cid: burnahCid
                    }))
                }
            }, 300);
...
$('.racing-map-delete-confirm').click(function () {  
    ...
    $.post('http://np-phone/racing:map:delete', JSON.stringify({ id: raceMap }));
    $('.racing-delete-confirm').fadeOut(150)
...
$("#phone-submit-number").click(function () {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            $.post("https://np-phone/getCallHistory", JSON.stringify({}))
        }, 300)
    }, 900)
})
...
$(".house-key-entries").on('click', '.h-removeKey', function (e) {
    ...
    $.post('http://np-phone/whatthefuckdoesthisshitwork', JSON.stringify({cid: overid, hid: overid2}));
})
//a-signin
...
$('.a-signin').click(function(){
    if (cuunt <= 0) {
     ...
     $.post('http://np-phone/abdulSignIn', JSON.stringify({status: taxiOn}));
 })
...
$(".house-entries, .access-house-entries").on('click', '.h-unlock', function (e) {
    ...
    $.post('http://np-phone/unlockHouses', JSON.stringify({hid: overid, penisfuck: hstatus}));
 })
 ...
 $(".house-entries").on('click', '.h-access-keys', function (e) {
    ...
    $("#house-2").hide();
    $('.access-house-entries').css('display', 'none');
    $.post('http://np-phone/btnKeyHolders', JSON.stringify({
        hid: overid
    }));
 ...
 $('.h-leaveedit').click(function(){
    editingHouse = false;
    $.post('http://np-phone/leaveEditMode', JSON.stringify({}))
    $("#house-2").show();
    $("#house-3").hide();
    $("#house-keys").hide();
    $(".btn-2").show();
 ...
 $('.h-openfurniture').click(function(){
    $.post('http://np-phone/openFurnitureUI', JSON.stringify({}))
 })

 $('.h-setbackdoor').click(function(){
    $.post('http://np-phone/setBackdoorUI', JSON.stringify({}))
 })

 $('.h-setwardrobe').click(function(){
    $.post('http://np-phone/setWardrobeUI', JSON.stringify({}))
 })

 $('.h-setcrafting').click(function(){
    $.post('http://np-phone/setCraftingUI', JSON.stringify({}))
 })

 $('.h-setgarage').click(function(){
    $.post('http://np-phone/setGarageUI', JSON.stringify({}))
 })

 $('.h-setstash').click(function(){
    $.post('http://np-phone/setStashUI', JSON.stringify({}))
 })
