            ...
            if currentDoorLockState ~= newLockState then
                if #(GetOffsetFromEntityGivenWorldCoords(PlayerPedId(), currentDoorCoords)) <= 1.2 then
                    ...
                    if hasAccess then
                        ...
                        exports["np-interface"]:showInteraction((newLockState and 'Locked' or 'Unlocked'), newLockState and 'error' or 'success')
                    end
                else
                    idle = 100
                end
...
RegisterNetEvent("np-doors:detCoord")
AddEventHandler("np-doors:detCoord", function ()
