...
Citizen.CreateThread(function()
    ...
    exports["np-polyzone"]:AddBoxZone("bowling_ballz", vector3(755.79, -774.93, 26.34), 1.6, 1.8, {
        name="bowling_ballz",
        heading=0,
        --debugPoly=false,
        minZ=23.94,
    ...
    exports["np-interact"]:AddPeekEntryByPolyTarget("bowling_ballz", {{
    ...
    for itemId, item in pairs(options) do
        if (itemId == 'bowlingreceipt') then
            ...
            data[#data + 1] = {
                ...
                key = itemId,
                children = {
                    { title = "Confirm Purchase", action = "np-ui:bowlingPurchase", key = itemId },
                },
            }
        end
    end
...
RegisterUICallback("np-ui:bowlingPurchase", function(data, cb)
    ...
    local success, message = RPC.execute("np-bowling:purchaseItem", data.key, isLane)
    if not success then
