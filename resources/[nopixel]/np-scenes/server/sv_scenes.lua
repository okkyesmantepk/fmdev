...
AddEventHandler("np-scenes:getScenes", function()
    ...
    TriggerClientEvent("np-scenes:allScenes", src, scenes)
end)

RegisterNetEvent("np-scenes:addScene")
AddEventHandler("np-scenes:addScene", function(pData)
    ...
    TriggerClientEvent("np-scenes:refreshScenes", -1, data[1].id, data[1])
end)
...
AddEventHandler("np-scenes:deleteScene", function(pCoords)
    ...
    for k,v in pairs(data) do
        ...
        if #(acCoord - pCoords) < 2.0 then
            ...
            if delete then
                TriggerClientEvent("np-scenes:deleteScene", -1, v.id)
                return
            end
        end
    end
