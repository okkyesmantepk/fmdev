...
RegisterNetEvent('np-jailmenu')
AddEventHandler('np-jailmenu', function()

	local Jailmenu = {
		{
            title = "List Lawyers",
            description = "Check how many lawyers are in the city!",
            action = "np-jail:lawyerspog",
        },
        {
            title = "Jail Time",
            description = "Check your jail time",
            action = "np-jail:checktime2",
        },
        {
            ...
            children = {
				{
                    title = "Yes",
                    action = "np-jail:swapchar",
                },
				{
                    title = "No",
                    action = "np-jail:swap_char_no",
                },
            },
        },
    }
    exports["np-interface"]:showContextMenu(Jailmenu)
end)

RegisterInterfaceCallback('np-jail:swap_char_no', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
end)

RegisterInterfaceCallback('np-jail:swapchar', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('swappingCharsLoop')
end)

RegisterInterfaceCallback('np-jail:checktime2', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('np-jail:checkTime')
end)

RegisterInterfaceCallback('np-jail:lawyerspog', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
