...
RegisterInterfaceCallback("np-healthcare:handler", function(data, cb)
		...
		while listening do
			if IsControlJustReleased(0, 38) then
				if pZone == "hospital_clothing_lockers_staff" then
					exports["np-interface"]:showContextMenu(zoneData[pZone].menuData)
				elseif pZone == "pillbox_checkin" or pZone == "viceroy_checkin" then
                    ...
                    local finished = exports["np-taskbar"]:taskBar(1500,string, false, true, false, false, nil, 5.0, PlayerPedId())
                    if finished == 100 then
							if pDoctors then
								TriggerEvent("DoLongHudText","A doctor has been paged. Please take a seat and wait.",2)
								TriggerEvent("np-jobs:EMS:doctorpaged", hospital)
							else
...
AddEventHandler("np-polyzone:enter", function(zone)
	...
	if currentZone then --and isCop
		...
		exports["np-interface"]:showInteraction(prompt)
...
AddEventHandler("np-healthcare:yoga", function(pArgs, pEntity, pContext)
	...
	until waitTime == 100 do
		...
		TriggerEvent("np-hud:updateStress", false, math.random(2,10))	
	end
end)
...
CreateThread(function()
	...
	exports["np-polyzone"]:AddBoxZone("infirm_char_switcher_backroom", vector3(1771.78, 2509.0, 45.83), 1.0, 1.6, {
		heading=301,
		minZ=44.88,
		maxZ=47.68
	})
