...
local function loopSkill(count)
    ...
    while loopCount < count do
        ...
        local finished = exports["np-ui"]:taskBarSkill(math.random(2000, 2500), math.random(7, 12))
...
local function reelInFish()
    ...
    for _, fish in pairs(fishes) do
        if not found and fish.chance > fishChance then
            ...
            if success then
                TriggerEvent('np-fishing:caught_fish')
            else
...
RegisterNetEvent('np-fishing:caught_fish')
AddEventHandler('np-fishing:caught_fish', function()
...
local function sellFish()
    ...
    if totalFish > 0 then
        if exports['np-hud']:MoneyBuff() then
            TriggerServerEvent('np-jobs:givePayout', totalFish * pricePerFish * 3, "cash", '[Fishing Job | With Buff]')
        else
            TriggerServerEvent('np-jobs:givePayout', totalFish * pricePerFish, "cash", '[Fishing Job | Without Buff]')
        end
...
RegisterNetEvent('np-fishing:clockIn')
AddEventHandler('np-fishing:clockIn', function()
    pClockedIn = true
    TriggerEvent('np-phone:signInFishing')
    TriggerEvent('DoLongHudText', '[ F1 > Blips > Fishing Location ]', 1)
end)

exports["np-polyzone"]:AddBoxZone("fishing",  vector3(-335.89, 6106.1, 31.45), 2, 2, {
    name = "fishing",
    heading=50,
    --debugPoly=false,
    minZ=28.45,
    ...
    Citizen.CreateThread(function()
        while sign_in_fishing do
			...
			if IsControlJustReleased(0, 38) then
               TriggerEvent('np-fishing:clockIn') 
			end
		end
	end)
end