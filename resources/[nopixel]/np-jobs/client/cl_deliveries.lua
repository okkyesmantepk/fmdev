...
RegisterNetEvent('np-jobs:deliverySignIn')
AddEventHandler('np-jobs:deliverySignIn', function()
    if not inJob then
        TriggerEvent('np-phone:signInInfoDelivery')
        print('[24/7 DELIVERIES] AWAITING JOB')
        inJob = true
        Citizen.Wait(math.random(60000, 120000))
        print('[24/7 DELIVERIES] SENDING JOB')
        TriggerEvent('np-phone:delivery_offer')
    end
end)

RegisterNetEvent('np-jobs:deliveries:getCar')
AddEventHandler('np-jobs:deliveries:getCar', function()
    local model = "benson"
    RequestModel(model)
    ...
    TriggerEvent('np-jobs:getDeliveryJob')
end)

RegisterNetEvent('np-jobs:deleteDeliveryVehicle')
AddEventHandler('np-jobs:deleteDeliveryVehicle', function()
    DeleteEntity(pVeh)
    pVeh = nil
end)

RegisterNetEvent('np-jobs:getDeliveryJob')
AddEventHandler('np-jobs:getDeliveryJob', function()
...
Citizen.CreateThread(function()
    exports["np-polyzone"]:AddBoxZone("delivery_store_1", vector3(-48.17, -1751.95, 29.42), 12.6, 15, {
    ...
    exports["np-polyzone"]:AddBoxZone("delivery_store_2", vector3(-712.37, -911.54, 19.01), 15, 10.2, {
    ...
    exports["np-polyzone"]:AddBoxZone("delivery_store_3", vector3(378.01, 329.12, 103.45), 12, 11.0, {
    ...
    exports["np-polyzone"]:AddBoxZone("delivery_store_4", vector3(-1224.38, -905.82, 12.31), 7, 7, {
    ...
    exports["np-polyzone"]:AddBoxZone("delivery_store_5", vector3(-2968.34, 390.36, 15.04), 7, 7, {
...
AddEventHandler('np-polyzone:enter', function(name)
    if name == "delivery_store_1" then
        ...
        if onWay1 and hasGoods then
            pDropGoods()
            exports['np-interface']:showInteraction('[E] Drop Goods [Delivery Job]')
            canDropGoods = true     
        end
	...
	Citizen.CreateThread(function()
        while delivery_store_1 or delivery_store_2 or delivery_store_3 or delivery_store_4 or delivery_store_5 do
			...
			if IsControlJustReleased(0, 38) and hasGoods then
                ...
                exports['np-interface']:hideInteraction()
                TriggerEvent('np-jobs:deliveryReturnToDepo')
			end
		end
	end)
end

RegisterNetEvent('np-jobs:deliveryReturnToDepo')
AddEventHandler('np-jobs:deliveryReturnToDepo', function()
    pDeliveryJob = false
    onWay1 = false
    onWay2 = false
    onWay3 = false
        ...
        local aDist = GetDistanceBetweenCoords(setCoords, coordA)
            if aDist < 15.0 then
                ...
                if exports['np-hud']:MoneyBuff() then
                    TriggerServerEvent('np-jobs:givePayout', math.random(650, 727), "cash", '[24/7 Delivery Job | With Buff]')
                else
                    TriggerServerEvent('np-jobs:givePayout', math.random(500, 678), "cash", '[24/7 Delivery Job | Without Buff]')
                end
                TriggerEvent('np-jobs:deleteDeliveryVehicle')
            end
        end
    end)
end)
...
exports["np-polytarget"]:AddCircleZone("delivery_start_stop",  vector3(919.82, -1256.64, 25.51), 0.8, {
    useZ = true
})

exports["np-interact"]:AddPeekEntryByPolyTarget("delivery_start_stop", {{
    event = "np-jobs:deliverySignIn",
...
-- Ped Shit -- (IGNORE THIS IS FOR np-ACTIVITIES HOUSE ROBBERIES)

exports["np-polytarget"]:AddCircleZone("start_house_robbery",  vector3(206.81, -1851.14, 27.76), 0.85, {
    useZ = true
})

exports["np-interact"]:AddPeekEntryByPolyTarget("start_house_robbery", {{
