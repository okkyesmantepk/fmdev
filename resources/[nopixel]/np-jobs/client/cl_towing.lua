...
AddEventHandler('np-jobs:tow_trucker_duty', function()
	if not clockedon then
		...
		clockedon = true
		TriggerEvent('np-phone:signInInfoTowing')
...
RegisterNetEvent('np-jobs:towing_accept')
AddEventHandler('np-jobs:towing_accept', function()
	GiveTowJobWanker()
end)
			...
			Citizen.CreateThread(function()
				while Nopixel.Looking do
					...
					if #(vector3(Nopixel.Location.x, Nopixel.Location.y, Nopixel.Location.z) - GetEntityCoords(PlayerPedId())) <= 320 then
						...
						setimpoundveh = plate
                        TriggerEvent('np-fuel:setFuel', GetVehiclePedIsIn(PlayerPedId()), 100)
...
AddEventHandler('np-jobs:give_tow_job', function()
	...
	if clockedon then
		TriggerEvent('np-phone:towing_offer')
	end
end)
...
Citizen.CreateThread(function()
    exports["np-polyzone"]:AddBoxZone("np_impound_location", vector3(-236.51, -1172.86, 37.6), 20, 27.6, {
...
AddEventHandler('np-jobs:impound_confirm_shit_idk_anymore', function()
	...
	if DoesEntityExist(vehicle) and NetworkHasControlOfEntity(vehicle) then
		...
		if licensePlate == setimpoundveh then
			...
			if exports['np-hud']:MoneyBuff() then
				TriggerServerEvent('np-jobs:givePayout', math.random(1076, 1554), "cash", '[24/7 Delivery Job | With Buff]')
			else
				TriggerServerEvent('np-jobs:givePayout', math.random(652, 886), "cash", '[24/7 Delivery Job | Without Buff]')
			end
			TriggerEvent('np-jobs:give_tow_job')
...
exports["np-polyzone"]:AddBoxZone("towing_shit", vector3(-191.63, -1166.46, 23.67), 2, 2, {
	name="towing_shit",
	heading=0,
	--debugPoly=false,
	minZ=21.27,
...
AddEventHandler('np-polyzone:enter', function(name)
	if name == "towing_shit" then
		...
		pSignOnOff()
		if not clockedon then
			exports['np-interface']:showInteraction('[E] Sign In')
		else
			exports['np-interface']:showInteraction('[E] Sign Out', 'error')
		end
	end
end)
	...
	Citizen.CreateThread(function()
        while towing_duty do
			...
			if IsControlJustReleased(0, 38) then
				if not clockedon then
					TriggerEvent('np-jobs:tow_trucker_duty')
					exports['np-interface']:hideInteraction()
				elseif clockedon then
					DeleteEntity(pVeh)
					TriggerEvent('np-jobs_tow_trucker_off_duty')
					exports['np-interface']:hideInteraction()
				end
			end
		end
