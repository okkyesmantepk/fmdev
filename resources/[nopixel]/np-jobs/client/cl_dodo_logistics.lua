...
RegisterNetEvent('np-jobs:passed')
AddEventHandler('np-jobs:passed', function()
    pisInDodoJob = true
    ...
    TriggerEvent('np-phone:goToLocation')
end)

RegisterNetEvent('np-jobs:startDodoLogistics')
AddEventHandler('np-jobs:startDodoLogistics', function()
    TriggerEvent('np-phone:signInDodo')
    ...
    TriggerEvent('np-phone:dodoJobOffer')
end)

RegisterNetEvent('np-jobs:dodoLogisticsPayout')
AddEventHandler('np-jobs:dodoLogisticsPayout', function()
    if pOnWayDodo == true then
        if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("pounder"))  then
            if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("pounder")) then
                TriggerServerEvent('np-jobs:dodoLogisticsPayout')
    ...
    while true do
        ...
        if isToDropLocation == true then
            ...
            if GetDistanceBetweenCoords(px,py,pz, GetEntityCoords(GetPlayerPed(-1),true)) < 2.5 then
                if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("pounder")) then
                    exports['np-interface']:showInteraction('[E] Drop Off Package')
                    if IsControlJustPressed(1,38) then
                        TriggerEvent('np-phone:goBackDepo')
                        exports['np-interface']:hideInteraction()
                            ...
                            local aDist = GetDistanceBetweenCoords(setCoords, coordA)
                                if aDist < 15.0 then
                                    ...
                                    if exports['np-hud']:MoneyBuff() then
                                        TriggerServerEvent('np-jobs:givePayout', math.random(878, 927), "bank", '[Dodo Logistics | With Buff]')
                                    else
                                        TriggerServerEvent('np-jobs:givePayout', math.random(650, 778), "bank", '[Dodo Logistics | Without Buff]')
                                    end
                                end
                            end
                        end)
...
RegisterNetEvent("np-jobs:spawn-truck")
AddEventHandler("np-jobs:spawn-truck", function()
	Citizen.CreateThread(function()
...
exports["np-polytarget"]:AddCircleZone("dodo_sign_in", vector3(-437.91, -2795.5, 7.39), 0.6, {
    useZ=true,
})

exports["np-interact"]:AddPeekEntryByPolyTarget("dodo_sign_in", {{
    event = "np-jobs:startDodoLogistics",
    id = "dodo_sign_in",
    icon = "circle",
    label = "Sign In",
    parameters = {},
