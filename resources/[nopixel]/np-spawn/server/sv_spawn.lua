...
RegisterServerEvent('np-spawn:createCharacter:log')
AddEventHandler('np-spawn:createCharacter:log', function(pFirstName, pSecondName, pDOB, pGender)
    local src = source
    local user = exports["np-base"]:getModule("Player"):GetUser(src)
    ...
    local connect = {
        {
          ["color"] = color,
          ["title"] = "**FM | Nopixel [np-spawn] | Create Character Log **",
...
RPC.register('np-spawn:isAdmin', function(src)
    local user = exports["np-base"]:getModule("Player"):GetUser(src)
