...
function Login.CreatePlayerCharacterPeds(characterModelData,isReset)
    ...
    TriggerEvent("np-weathersync:client:DisableSyncSpawn")
...
exports["np-polyzone"]:AddBoxZone("scuff_shit", vector3(-3.17, 1.97, 1.47), 15, 15, {
    heading=0,
    debugPoly=false,
    minZ=0.07,
    maxZ=4.07
...
AddEventHandler('np-polyzone:enter', function(name)
    if name == "scuff_shit" then
        pInZone = true
        exports["np-interface"]:showInteraction("[E] Information")
        teleport_to_coords()
    end
end)
