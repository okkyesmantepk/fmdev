...
AddEventHandler("np-spawn:firstspawnyo", function()
	if triggered then
		triggered = false
		if not exports["np-inventory"]:hasEnoughOfItem("newaccountbox",1,false) then
...
AddEventHandler('np-spawn:finishedClothingstuffbish', function()
	...
	if not exports["np-inventory"]:hasEnoughOfItem("lockpick",1,false)then
	...
	if not exports["np-inventory"]:hasEnoughOfItem("water",1,false)then
...
function Spawn.getRoosterSpawn()
    ...
    local isRooster = exports["np-business"]:IsEmployedAt("rooster")
	if isRooster then
