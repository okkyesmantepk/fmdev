...
function ChopVehicleRemains(vehicle, boneID)
    ...
    TriggerEvent('np-chop:cooldown')
    return success
end

RegisterNetEvent('np-chop:cooldown')
AddEventHandler('np-chop:cooldown', function()
    CoolDownActive = true
    Citizen.Wait(math.random(60000, 120000))
    CoolDownActive = false
    if ClockedIntoChop then
        TriggerEvent('np-phone:chop_shop_offer')
        CreateChopCar()
    end
end)
...
AddEventHandler('np-chopshop:clock-in', function()
    ...
    elseif ClockedIntoChop then
        TriggerEvent('np-chopshop:clock-out')
    else
        ClockedIntoChop = true
        TriggerEvent('np-phone:signInInfoChopshop')
        Citizen.Wait(math.random(60000, 120000))
        TriggerEvent('np-phone:chop_shop_offer')
    end
end)
...
function CreatePickupCarBlip(x, y ,z)
  ...
  RegisterNetEvent('np-chopshop:acceptOffer')
  AddEventHandler('np-chopshop:acceptOffer', function()
    CreateChopCar()
  end)
