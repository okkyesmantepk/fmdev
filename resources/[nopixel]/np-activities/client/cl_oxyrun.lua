Config = {
    ...
    local bliptable = {}
        ...
        RegisterNetEvent('start-oxy', function()
            if exports['np-inventory']:hasEnoughOfItem('darkmarketdeliveries', 1) and exports['np-inventory']:hasEnoughOfItem('vpnxj', 1) then
                TriggerEvent('np-phone:oxyRunNoti', 'Wait for a job offer !')
                Citizen.Wait(math.random(60000, 120000))
                TriggerEvent('np-phone:oxyrun:offer')
            else
                TriggerEvent('DoLongHudText', 'You need a delivery list and a VPN to work here !', 2)
            end
        end)

        RegisterNetEvent('np-oxy:Accept:Offer')
        AddEventHandler('np-oxy:Accept:Offer', function()
            TriggerServerEvent('started-job')
            TriggerEvent('np-phone:oxyRunNoti', 'Head towards the location on your GPS.')
        end)
        ...
        RegisterNetEvent('generate-ped-loc', function()
            ...
            exports["np-interact"]:AddPeekEntryByModel(GetHashKey('a_m_m_bevhills_01'), {{
        ...
        RegisterNetEvent('collect-package', function()
            if toplampaket < 10 then
                ...
                if not exports['np-inventory']:hasEnoughOfItem('darkmarketpackage', 1) then
                    ...
                    TriggerEvent('np-phone:oxyRunNoti', '('..toplampaket..'/10) Packages Collected.')

                    if toplampaket == 10 then
                        TriggerEvent('np-phone:oxyRunNoti', 'All Packages Collected.')
        ...
        RegisterNetEvent('generate-new-corner-loc', function()
            ...
            jobs = jobs+1
            TriggerEvent('np-phone:oxyRunNoti', 'Drive to the next location.')
            ...
            if exports['np-inventory']:hasEnoughOfItem('darkmarketpackage', 1) then
                if licensePlate == pPlate then
                    ...
                    if handoff < 5 then
                        ...
                        handoff = handoff+1
                        TriggerEvent('np-phone:oxyRunNoti', '('..handoff..'/5) Packages Handed Off.')
                        ...
                        if exports['np-inventory']:hasEnoughOfItem('darkmarketpackage', 1, true) then
                            local rnd = math.random(1, 2)
                            if rnd == 1 then
                                exports["np-dispatch"]:dispatchadd('10-37', "Investigate Suspicious Activity", "102")
                                TriggerEvent("player:receiveItem", "oxy", math.random(1, 5))
                            end
                            TriggerEvent("inventory:removeItem", "darkmarketpackage", 1)
                            TriggerEvent('np-oxyruns:clean')
                            Wait(500)
                            TaskVehicleDriveWander(ped, veh, 20.0, 16777216)
                            SetDriverAggressiveness(ped, 0.0)
                            Wait(15000)
        ...
        AddEventHandler("np-oxyruns:clean", function(pRandomChance)
            local payment = 0
            for typ, conf in pairs(billsCleaningStuff) do
              ...
              if math.random() < randomChance and exports["np-inventory"]:hasEnoughOfItem(typ, totalAmount, false) then
                ...
                if exports['np-hud']:MoneyBuff() then
                    TriggerServerEvent('np-activities:givePayout', payment*2, 'cash', '[Oxy Run | With Buff | Cleaned Bands]')
                else
                    TriggerServerEvent('np-activities:givePayout', payment, 'cash', '[Oxy Run | Without Buff | Cleaned Bands]')
                end
              end
            end
            if payment == 0 then
                TriggerServerEvent('np-activities:givePayout', math.random(150, 350), 'cash', '[Oxy Run | Without Buff | Reg Payout]')
            end
          end)
          ...
          function thread()
            ...
            if Config.TargetName == 'q' then
                ...
                exports["np-polytarget"]:AddCircleZone("oxystart", vector3(-1564.06, -440.33, 37.39), 0.55, {
                    useZ=true,
                })
                
                exports["np-interact"]:AddPeekEntryByPolyTarget("oxystart", {
                    {
                        event = "start-oxy",
                        id = "oxystart1",
                        icon = "circle",
