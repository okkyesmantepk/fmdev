...
AddEventHandler('RunUseItem', function(itemid, slot, inventoryName, isWeapon, passedItemInfo)
    ...
    if itemid == "chicken_noodle_soup" then
        ...
        if finished == 100 then
            ...
            TriggerEvent('np-buffs:strength_buff')
    ...
    if itemid == "doki_doki_pancakes" then
        ...
        if finished == 100 then
            TriggerEvent('np-hud:ChangeHunger', 50)
            TriggerEvent('np-buffs:money_boost_buff')
    ...
    if (itemid == "bubbletea") then
        ...
        TriggerEvent('np-hud:ChangeThirst', 30)
        TriggerEvent('np-buffs:bulb_buff')
    end
    ...
    if itemid == "fries" then
        ...
        TriggerEvent('np-hud:ChangeHunger', 30)
        TriggerEvent('np-buffs:swim_buff')
    end
    ...
    if (itemid == "spraycan") then
        local ItemInfo = GetItemInfo(slot)
        local data = json.decode(passedItemInfo)
        TriggerEvent('np-gang-sprays:startPlacing', json.decode(ItemInfo.information).model, slot)
    end
...
AddEventHandler('inv:lockPick', function(isForced,inventoryName,slot)
    ...
    if targetVehicle == 0 then
        ...
        if driverPed ~= 0 then
            ...
            exports['np-dispatch']:dispatchadd('10-99B', 'Stolen Vehicle', '227')
...
RegisterInterfaceCallback("np-ui:inv:moneycaseAction", function(data, cb)
...
RegisterInterfaceCallback('np-ui:cancelNotepadEmote', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent("animation:PlayAnimation", "cancel")
end)

RegisterInterfaceCallback('np-ui:createNotepadNote', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
