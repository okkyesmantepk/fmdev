    ...
    for ind, value in pairs(data) do
        ...
        local house = exports["np-menu"]:NearHouseGarage()
            if house then
                ...
                SetFitment = true
                TriggerEvent('np-garages:fitmentSet')
            end
        else
    ...
    for ind, value in pairs(data) do
        ...
        local house2 = exports["np-menu"]:NearHouseGarage2()
            if house2 then
                ...
                TriggerServerEvent('np-wheelfitment:GetVehicleStatus', GetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId(), false)), GetVehiclePedIsIn(PlayerPedId(), false))
                SpawnVehicle(value.model, exports['np-menu']:currentGarage(), value.fuel, value.data, value.license_plate, true)
                pPlate = value.license_plate
                SetFitment = true
                TriggerEvent('np-garages:fitmentSet')
...
RegisterNetEvent("np-garages:store", function()
    ...
    local get_plate = exports["np-garages"]:NearVehicle("plate")

    if (curVeh ~= 0) then
        local Stored = RPC.execute("np-garages:states", "In", get_plate, exports['np-menu']:currentGarage(), exports["np-garages"]:NearVehicle("Fuel"))
        if Stored then
...
RegisterNetEvent('np-garages:fitmentSet')
AddEventHandler('np-garages:fitmentSet', function()
    Citizen.CreateThread(function()
        while SetFitment do
            Citizen.Wait(1000)
            if SetFitment then
                if GetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId())) == pPlate then
                    if IsPedInAnyVehicle(PlayerPedId()) then
                        SetFitment = false
                        TriggerServerEvent('np-wheelfitment:GetVehicleStatus', GetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId(), false)),  GetVehiclePedIsIn(PlayerPedId()))
                    end
                end
            end
        end
...
RegisterNetEvent('np-garages:pd_heli')
AddEventHandler('np-garages:pd_heli', function()
    local PDHeliGarage = {
        {
            ...
            children = {
                ...
                children = {
                    title = "Take Out",
                    action = "np-garages:spawn_heli"
                }
            }
        },
    }
    exports["np-interface"]:showContextMenu(PDHeliGarage)
end)

RegisterInterfaceCallback('np-garages:spawn_heli', function(data, cb)
