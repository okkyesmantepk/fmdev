local LAUNDRY_COORDS = {
  ...
  AddEventHandler("np-casino:hotel:doLaundry", function(p1, p2, pContext)
    ...
    local enough = exports["np-inventory"]:hasEnoughOfItem("markedbills",5,false)

    if enough then

    local canLaunder = RPC.execute("np-casino:hotel:canUseLaundry", laundryId, true)
    ...
    RPC.execute("np-casino:hotel:finishup", laundryId , {} )
    end)
else
