...
function dwStatusHud(pValues)
  ...
  exports["np-interface"]:sendAppEvent("status-hud", {
    show = true,
    title = title,
    values = values,
  })
--   SendNUIMessage({ -- If you don't have np-interface
--     app = "status-hud",
--     show = false,
--     data = {
--         show = true,
            ...
            if closestChair ~= nil and closestChairDist < 1.6 then
                if not timeoutHowToBlackjack then
                    if not uiIntShown then
                        ...
                        -- exports["np-interface"]:showInteraction(
                        --   "[E] Play! " .. dwGetLimitTitleString(),
                        --   background
                        -- )
                        exports["np-interface"]:showInteraction("[E] Play! " .. dwGetLimitTitleString() , background)
                        uiIntShown = true
                    end
                end
...
RegisterInterfaceCallback("np-interface:blackjackHitStand", function(data, cb)
...
Citizen.CreateThread(function()
    while true do
        ...
        --         data[#data + 1] = {
        --           action = "np-interface:blackjackHitStand",
...
RegisterInterfaceCallback("np-interface:blackJackBet", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
...
AddEventHandler("np-interface:application-closed", function(name, data)
...
function dwWaitForBet(act)
  ...
  if waitingForBetState then
    ...
    if sittingAtBlackjackTable then
      ...
      local data = {}
      data[#data + 1] = {
        action = "np-interface:blackJackBet",
        title = "Place bet",
