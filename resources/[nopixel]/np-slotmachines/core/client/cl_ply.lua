        ...
        while true do
          ...
          if not isNear(plyPos, v.chairPos, 0.75) then
            if isInteractionShowing then
              ...
              exports["np-interface"]:hideInteraction(interactionBackground)
            end
          ...
          if not isInteractionShowing then
            ...
            exports["np-interface"]:showInteraction("[E] Play " .. v.label, interactionBackground)
          end
...
--#[NUI Callbacks]#--
RegisterInterfaceCallback("np-textui-slotmachines:placeBet", function(data, cb)
...
RegisterInterfaceCallback("np-textui-slotmachines:leaveMachine", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  getUpFromSlotMachine()
  SendNUIMessage({
    app = "status-hud",
