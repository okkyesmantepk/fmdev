...
function IsTunerWorker()
    local mypasses = exports['np-business']:IsEmployedAt('tuner_shop')
...
function IsHayesWorker()
    local mypasses = exports['np-business']:IsEmployedAt('hayes_autos')
        if mypasses then
            return true
        end
    return false
...
AddEventHandler("bennys:enter", function()
    ...
    elseif currentBennys == "bennyshayes" then
        if exports['np-business']:IsEmployedAt('hayes_autos') then
            enterLocation(bennys, repair)
        end
    ...
    elseif currentBennys == "bennysharmony" then
        if exports['np-business']:IsEmployedAt('harmony_repairs') then
            enterLocation(bennys, repair)
        end
    elseif currentBennys == "ottosbay1" then
        if exports['np-business']:IsEmployedAt('ottos_autos') then
            enterLocation(bennys, repair)
        end
...
RegisterNetEvent("np-bennys:repairVeh")
AddEventHandler('np-bennys:repairVeh', function()
...
AddEventHandler('np-public-bennys', function()
    TriggerServerEvent('np-bennys:payPublicBennys')
end)

Citizen.CreateThread(function()
    exports["np-polyzone"]:AddBoxZone("pubbennys", vector3(-33.2, -1053.37, 28.4), 8.0, 9.4, {
        heading=340,
        minZ=25.72,
        maxZ=29.52
    })
