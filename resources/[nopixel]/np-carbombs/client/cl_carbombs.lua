...
local function checkForCarBomb(pEntity) 
  ...
  local progress = exports['np-taskbar']:taskBar(2500, 'Searching for car bomb...', true)
  ClearPedTasks(PlayerPedId())

  if progress ~= 100 then return end

  local metaData = RPC.execute("np-miscscripts:carbombs:getCarBombDataFromNetID", NetworkGetNetworkIdFromEntity(pEntity))
...
--[[   local remoteBomb = RPC.execute('np-usableprops:hasPhoneBomb', NetworkGetNetworkIdFromEntity(pEntity))
...
RegisterNetEvent("np-miscscripts:carbombs:foundBombAll", function(pNetTable)
...
AddEventHandler("np-inventory:itemUsed", function(name, info)
  ...
  local prompt = exports['np-interface']:OpenInputMenu(elements)
  ...
  local progress = exports['np-taskbar']:taskBar(2500, 'Planting car bomb...', true)
  ...
  local added = RPC.execute('np-miscscripts:carbombs:addCarBomb', netId, minSpeed, ticksBeforeExplode, ticksForRemoval, gridSize, 0, timeToComplete)
...
AddEventHandler('np-miscscripts:carBombs:removeBomb', function (pData, pEntity)
  ...
  if carBombMeta and carBombMeta.hasCarBomb then
    ...
    TriggerEvent('animation:PlayAnimation', 'kneel')
    TriggerServerEvent('np-miscscripts:carbombs:removeBomb', myNetIdentifier)
