...
function HotwireVehicle(cb)
    ...
    for i = 1, 5 do
        ...
        local finished = exports["np-ui"]:taskBarSkill(skill[1],  skill[2])
            ...
            if not HasVehicleKey(vehicle) then
                ...
                if pedDriver ~= 0 and (not IsPedAPlayer(pedDriver) or IsEntityDead(pedDriver)) then
                    if IsEntityDead(pedDriver) then
                        ...
                        local finished = exports["np-taskbar"]:taskBar(3000, "Pegando Chaves", false)
                        if finished == 100 then
                        ...
                        if math.random(100) > 95 then
                            ...
                            local finished = exports["np-taskbar"]:taskBar(3000, "Pegando Chaves")
                            if finished == 100 then
                                TriggerEvent("keys:addNew", vehicle)
                            else
                                ClearPedTasksImmediately(PlayerPedId())
